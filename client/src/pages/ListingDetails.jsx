...
const ListingDetails = () => {
  ...
  const purchaseAccount = async () => {
    ...
    try {
      ...
      const { data } = await api.post(`/api/listing/purchase-account/${listing.id}`,{} ,{ headers: { Authorization: `Bearer ${token}` } })
  ...
  useEffect(() => {
    const listing = listings.find((lt) => lt.id === listingId)
    if (listing) {
      setListing(listing)
    }
  }, [listingId, listings])
    ...
    <div className="mx-auto min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
      ...
      <div className="flex items-start max-md:flex-col gap-10">
            ...
            <div className="bg-white rounded-xl border border-gray-200 mb-5 overflow-hidden">
              ...
              <div className="relative w-full aspect-video overflow-hidden">
                ...
                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2">
                  {images.map((_, index) => (
                    <button onClick={() => setCurrent(index)} key={index} className={`w-2.5 h-2.5 rounded-full ${current === index ? "bg-indigo-600" : "bg-gray-200"}`} />
                  ))}
                </div>
              </div>
        ...
        <div className="bg-white min-w-full md:min-w-[370px] rounded-xl border border-gray-200 p-5 max-md:mb-10">
          ...
          <button className="w-full bg-indigo-600 text-white py-2 rounded-lg hover:bg-indigo-700 transition text-sm font-medium flex items-center justify-center gap-2" onClick={loadChatbox}>
            <MessageSquareMoreIcon className="size-4" /> Chat
          </button>
          {
            listing.isCredentialChanged && (
              <button className="w-full mt-2 bg-purple-600 text-white py-2 rounded-lg hover:bg-purple-700 transition text-sm font-medium flex items-center justify-center gap-2" onClick={purchaseAccount}>
