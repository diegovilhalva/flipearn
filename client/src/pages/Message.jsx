    ...
    <div className="mx-auto min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
      <div className="py-0">
        ...
        <div className="relative max-w-xl mb-8">
          ...
          <input type="text" placeholder='Search conversations' className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-indigo-500" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>
        {loading ? (
          <div className="text-center text-gray-500 py-20">Loading messages</div>
        ) : filteredChats.length === 0 ? (
          <div className="bg-white rounded-lg shadow-xs border border-gray-20 p-16 text-center">
            ...
            <p className='text-gray-600'>
              {searchQuery ? "Try a diffrent search term" : "Start a conversation by viewing a listing and clicking 'Chat with Seller'"}
            </p>
          </div>
        ) : (
          <div className="bg-white rounded-lg shadow-xs border border-gray-200 divide-y divide-gray-200">
            {filteredChats.map((chat) => {
              const chatUser = chat.chatUserId === user?.id ? chat.ownerUser : chat.chatUser
              return (
                <button key={chat.id} className="w-full p-4 hover:bg-gray-50 transition-color text-left" onClick={() => handleOpenChat(chat)}>
