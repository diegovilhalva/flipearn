import { Badge, BadgeCheck, LineChart, MapPin, User } from "lucide-react"
    ...
    return (
        <div className="relative bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition">
            {listing.featured && (
                <>
                    <p className="py-1" />
                    <div className="absolute top-0 left-0 w-full bg-linear-to-r from-pink-500 to-purple-500 text-white text-center text-xs font-semibold py-1 tracking-wide uppercase">Featured</div>
                </>
            )}
            <div className="p-5 pt-8">
                ...
                <div className="flex flex-wrap justify-between max-w-lg items-center gap-3 my-5">
                    <div className="flex items-center text-sm text-gray-600">
                        ...
                        <span className="text-lg font-medium text-salte-800 mr-1.5">{listing.followers_count.toLocaleString()}</span> followers
                    </div>
                    {
                        listing.engagement_rate && (
                            <div className="flex items-center text-sm text-gray-600">
                                <LineChart />
                                <span>{listing.engagement_rate}</span> % engagement
                            </div>
                        )
                    }
                </div>
