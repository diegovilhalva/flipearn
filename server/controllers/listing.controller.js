...
export const addListing = async (req, res) => {
    try {
        ...
        const uploadImages = req.files.map(async (img) => {
        ...
        return res.status(201).json({ message: "Acount Listed sucessfully", listing })
...
export const updateListing = async (req, res) => {
    try {
        ...
        if (req.files.length + accountDetails.images.length > 5) {
            return res.status(400).json({ messages: "You can only upload up to 5 images" })
        }
...
export const toggleStatus = async (req, res) => {
    try {
        ...
        return res.json({ message: "Listung status updated succesfully", listing })
    } catch (error) {
        console.log(error)
        res.status(500).json({ message: error.code || error.message })
    }
...
export const deleteUserListing = async (req, res) => {
    try {
        ...
        if (listing.isCredentialChanged) {
            await inngest.send({name:"app/listing-deleted",data:{listing,listingId}})
        }
        ...
        return res.json({ message: "Listing deletd successfully" })
    } catch (error) {
        console.log(error)
        res.status(500).json({ message: error.code || error.message })
    }
...
export const addCredential = async (req, res) => {
    try {
        ...
        return res.json({ message: "Credential added sucessfully" })
...
export const widthDrawnAmount = async (req, res) => {
    try {
        ...
        return res.json({ message: "Applied for withdrawal", withdrawn })
    } catch (error) {
        console.log(error)
        res.status(500).json({ message: error.code || error.message })
    }
